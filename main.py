...
import emotion_identifier as em
import av
from PIL import Image
import numpy as np 
...
class VideoProcessed:
    ...
    def recv(self,frame):
        ...
        fem , details = em.Face_analyzer(frm)
        try:
            st.write(details['dominant_emotion'])
        except:
            pass
        
        return av.VideoFrame.from_ndarray(fem, format='bgr24')
...
#==========================================================================================================
st.title("RevealEmotion")
input_image = []
photo=[]
optionlists=[]
with st.sidebar:
#photo = st.sidebar.file_uploader("Choose Image" , type = ['png' , 'jpeg' , 'png' , 'jpg'])
    toollist = st.sidebar.selectbox("Choose one" , 
                              ["Photo Editor" ,
                               "Emotion Detector" ] )
    if(toollist == "Photo Editor"):
        photo = st.sidebar.file_uploader("Choose Image" , type = ['png' , 'jpeg' , 'png' , 'jpg'])
    if(toollist == "Emotion Detector"):
        optionlists = st.sidebar.radio("Slider" , 
                                        ["Live Video" ,
                                        "Image upload" ] )    
...
if (optionlists == "Live Video"):
    webrtc_streamer(key = "key" , video_processor_factory=VideoProcessed,
                    rtc_configuration=RTCConfiguration(
					{"iceServers": [{"urls": ["stun:stun.l.google.com:19302"]}]})
                    )
    try:
        st.sidebar.write(VideoProcessed.result['dominant_emotion'])
    except:
        pass

if(optionlists == "Image upload"):
    photos = st.sidebar.file_uploader("Choose Image" , type = ['png' , 'jpeg' , 'png' , 'jpg'])
    if(photos):
        res,a = em.Face_analyzer(np.array(Image.open(photos)))
        st.image(res)
